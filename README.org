...
*Scope*: *This file* exports everything; *Choose a heading…* shows every heading in the document — regardless of current fold state, so something folded away is still pickable — and exports just that heading and its descendants. The selected heading becomes the new top level (~#~ / ~<h1>~) in the output, not its original depth, so a level-3 heading exported on its own doesn't produce a dangling ~###~ / ~<h3>~ with nothing above it. Calendar export shares this same scope choice too (see below) — including its own *Choose a heading…* option, exporting just that heading's dated items and its descendants'.
