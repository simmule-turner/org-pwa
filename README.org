...
- [[*What org-pwa is (and isn't)][What org-pwa is (and isn't)]]
- [[#named-target][Custom Target Anchor]]
...
Every delete action still asks for confirmation — but is no longer irreversible: *Undo/redo now covers every edit in this app* (see [[*Undo/redo and History][Undo/redo and History]]).
...
This documentation *is* an org file (~README.org~), rendered by this app's own org parser and outline renderer — the same engine every other file goes through, not a separate parser built just for docs. Folding works exactly like the main outline (tap a chevron to collapse/expand a section), and the table of contents' internal links are real org links (~[[*Section Name]]~) that jump straight to the target section, auto-expanding it if it was folded. **Read-only, deliberately**: nothing here is editable — no tap-to-edit, no action menus — and viewing it never touches whatever file you actually have open elsewhere in the app; switching back and forth loses no state and prompts for nothing. External links (anything not starting with ~[[*~) open in a real new tab.
