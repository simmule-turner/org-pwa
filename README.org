...
- ~prepend~ /(optional, default ~false~)/ — real org's own ~:prepend t~ property: puts the new entry at the *top* of the target instead of the bottom. Works for all four types — a new list item lands before the first existing item, a new heading (~plain~ type) before the first existing subheading, a new table row right after any header row + rule line (so it becomes the first *data* row, not literally above the header). A brand-new table (nothing to prepend relative to yet) behaves identically either way.
