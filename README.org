...
*org-clock-out* replaces that bare line with the completed form once you tap it again: ~CLOCK: [start]--[end] =>  H:MM~, computing the duration directly from the two actual timestamps. Tapping Clock out with nothing running is also a no-op.
...
*What's not built*: ~org-clock-cancel~ (abandoning a running clock without recording it — clocking out is the only way to stop one, always recording whatever time elapsed); a persistent mode-line/status-bar display of the currently-running clock; ~org-clock-report~ / clock tables (~#+BEGIN: clocktable~ dynamic blocks); clock-continue (resuming the most recently-clocked task); and effort estimates / clock-in reminders tied to them. See [[#differences-from-emacs-org-mode][Differences from Emacs org-mode]].
...
- A *quoted function reference* (~'org-clock-out~ above) — selecting the item runs that specific built-in function directly. Only ~org-clock-out~ is supported today (clocks out of whichever heading currently has a running clock, wherever it is in the document — see [[#clocking][Clocking]]; a status message shows if nothing is running); more may be added over time as they become useful for a quick-access menu specifically.
...
*Clocking* (see [[#clocking][Clocking]] for the full behavior) covers the core ~org-clock-in~ / ~org-clock-out~ / ~org-clock-display~ commands. What's different from real Emacs: no ~org-clock-cancel~, no persistent mode-line display of the running clock, no ~org-clock-report~ / clock tables (~#+BEGIN: clocktable~), no clock-continue, and no effort estimates.
...
- No ~org-clock-cancel~, persistent mode-line clock display, ~org-clock-report~ / clock tables, clock-continue, or effort estimates — see [[#clocking][Clocking]] for what IS built (~org-clock-in~ / ~org-clock-out~ / ~org-clock-display~); no fast-key-driven jump-to-state UI for progress logging's own per-keyword spec either, tap-to-cycle is still the only way to change state
- ~org-extra-menu~'s function-reference entries only recognize ~'org-clock-out~ today — an unrecognized function name is silently skipped rather than an error, so more can be added over time without breaking anything already configured — see [[#extras-menu][Extras menu]]
