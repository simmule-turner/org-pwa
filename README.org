...
| ~org-archive-confirm~ | ~t~ | Whether archiving/unarchiving asks for confirmation first (not a real org-mode variable -- see below) |
...
- ~org-archive-confirm~ — *not actually a real org-mode variable, and it would be misleading to present it as one*: made up for this app specifically — Emacs org-mode has no variable by this name, and archiving in real Emacs doesn't ask for confirmation at all. It piggybacks on the same Local/Global Variables mechanism purely as a convenient, already-existing place to put a per-file setting, not because it's reproducing something org-mode itself does. ~t~ or ~nil~; ~t~ (the default here) shows the Archive/Refile/Cancel/OK prompt naming the actual destination before archiving or unarchiving a heading; ~nil~ skips the prompt and archives immediately (Refile remains available as its own separate action either way — this only controls whether Archive itself prompts first). See [[#archiving][Archiving]].
...
*Confirmation*: with ~org-archive-confirm~ at its default ~t~ — an app-specific setting, not a real org-mode variable (see [[#local-variables][Local Variables]] for why it's presented this way regardless) — tapping *Archive* shows a prompt naming the actual destination, with three choices rather than a plain OK/Cancel: *Refile…* opens the [[#refile][Refile]] picker instead, for when the actual destination isn't the archive location specifically; *Cancel* leaves everything untouched; *OK* proceeds with the archive exactly as described above. Once the move actually completes, the status line reads ~--- Archive complete.~ Set ~org-archive-confirm~ to ~nil~ to skip the prompt and archive immediately — Refile remains available as its own separate action from the same heading either way, this setting only controls whether Archive itself prompts first.
...
Tap a heading's title, then *Archive*, then *Refile…* on the confirmation prompt (see [[#archiving][Archiving]]) — not a separate action-menu button of its own — to move the whole subtree somewhere else in the outline (or a different file entirely): real org's own ~org-refile~. Only reachable when ~org-archive-confirm~ is on (its default) and the heading isn't currently archived in place, since Archive's own confirm prompt is the only entry point.
