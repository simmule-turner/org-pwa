...
- *The general editor* (📋 *Edit details*): one structured form covering everything that used to be spread across several separate actions:
  ...
  All fields across all four sections are committed together on *Save*, or discarded together on *Cancel* — nothing is written until you actually save. A separate *▦ Add table* button sits above Save/Cancel — an immediate, standalone action (inserts a table into the heading's body right away, closing the editor) rather than one of the four fields committed together, since it doesn't interact with any of scheduling/tags/priority/properties at all. When not editing, existing properties still show as a small read-only line under the heading, but only when drawers are actually visible for that heading (~#+STARTUP: showeverything~, or manually cycled to "fully expanded" — see [[#folding-and-startup][Folding and #+STARTUP]]); tapping it opens the same general editor as tapping the title. Collapsing the heading again correctly hides them too, whether via swipe or chevron.
...
- A *quoted function reference* (~'org-clock-out~ above) — selecting the item runs that specific built-in function directly. ~org-clock-out~ and ~org-clock-cancel~ are supported today (both act on whichever heading currently has a running clock, wherever it is in the document — see [[#clocking][Clocking]] for the difference between the two; a status message shows if nothing is running). Selecting either runs it directly, no additional confirmation prompt — unlike the action menu's own Clock out button, which now asks first (Cancel/Stop/OK); a deliberate menu selection is already the confirming step. More functions may be added over time as they become useful for a quick-access menu specifically.
...
*File → Export…* gets a document out of org-pwa as ASCII, Markdown, HTML, or a calendar file, for the whole file or (ASCII/Markdown/HTML) just one heading's subtree. All four walk the same parsed structure the app already renders from, not a text-level reparse.

*Scope*: *This file* exports everything; *Choose a heading…* shows every heading in the document — regardless of current fold state, so something folded away is still pickable — and exports just that heading and its descendants. The selected heading becomes the new top level (plain text / ~#~ / ~<h1>~, depending on format) in the output, not its original depth, so a level-3 heading exported on its own doesn't produce a dangling, orphaned sub-level with nothing above it. Calendar export shares this same scope choice too (see below) — including its own *Choose a heading…* option, exporting just that heading's dated items and its descendants'.
...
- ~org-extra-menu~'s function-reference entries only recognize ~'org-clock-out~ and ~'org-clock-cancel~ today — an unrecognized function name is silently skipped rather than an error, so more can be added over time without breaking anything already configured — see [[#extras-menu][Extras menu]]
