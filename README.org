...
*File → Export…* gets a document out of org-pwa as ASCII, ODT, HTML, or a calendar file, for the whole file or (ASCII/ODT/HTML) just one heading's subtree. All four walk the same parsed structure the app already renders from, not a text-level reparse. Markdown export still exists and is fully supported — see the *Markdown* paragraph below for how to reach it now that ODT has taken its place as the main menu's word-processor-format option.
...
*Export* (see [[#export][Export]] for the full behavior) covers ASCII, Markdown, HTML, and Calendar (.ics) — the whole file or (ASCII/Markdown/HTML) a single heading's subtree — with PDF available via the browser's own print dialog on the exported HTML rather than a separate hand-rolled PDF path. What's different from real org's own export backends: no LaTeX/ODT/other document formats; internal links aren't resolved to anchors in Markdown/HTML output (the raw org target is kept as the link's href verbatim, rather than guessing at a heading-slug algorithm the eventual renderer might not actually match); and ASCII export doesn't implement ~ox-ascii~'s fuller feature set (section numbering, a generated table of contents, Latin-1/UTF-8-specific character substitutions) — a plain, readable, width-wrapped rendering of the outline, not full fidelity with real org's own backend.
...
- No LaTeX/ODT/other document export formats beyond ASCII, Markdown, and HTML (PDF is covered via the browser's own print dialog on the exported HTML, see [[#export][Export]]); internal links aren't resolved to anchors in Markdown/HTML output; ASCII export doesn't implement real org's ~ox-ascii~ fuller feature set (section numbering, a generated table of contents, Latin-1/UTF-8-specific character substitutions)
