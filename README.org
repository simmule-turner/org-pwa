...
2. *~#+STARTUP:~* ([[#folding-and-startup][below]]) -- a file's own directive line, overriding the global baseline for that file specifically. Only a handful of variables have a ~#+STARTUP:~ shorthand at all (heading visibility, inline images, and ~org-log-done~ are the ones this app recognizes) -- most variables skip this layer entirely and are only ever set via Global Variables or Local Variables.
...
- No clocking (~org-clock-in~ / ~-out~ / ~-cancel~ / ~-display~) — ~CLOCK:~ lines round-trip cleanly but nothing starts, stops, or displays one; no fast-key-driven jump-to-state UI for progress logging's own per-keyword spec either, tap-to-cycle is still the only way to change state
