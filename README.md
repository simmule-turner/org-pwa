...
**Single document at a time.** Opening a different document — via a `file:`/`github:`/`webdav:` link, or File → Open — always replaces what's currently showing; there's no true simultaneous multi-document editing (several documents open/dirty/rendered at once, tab-bar style), by design, matching this app's own single-scrollable-pane philosophy elsewhere. `org-agenda-files` (see [Agenda](#agenda)) and capture-to-file (see [Capture Templates](#capture-templates)) both work across multiple files without this limitation applying to them at all, since neither needs its target file to become the active one.
...
- No dedicated "browse your recently-opened documents" switcher UI — switching between documents happens by tapping a `file:`/`github:`/`webdav:` link (see [Links and images](#links-and-images)) or File → Open, not a tab bar or history list. There's also no true simultaneous multi-document editing — exactly one document is ever open/dirty/rendered at a time, by design (see [Platform support](#platform-support)'s screen-size notes for the same "one active thing at a time" philosophy applied elsewhere in this app). Capture templates and the Agenda/TODO views (see [Capture Templates](#capture-templates), [Agenda](#agenda)) both work across multiple files without needing a switcher at all, matching how real org itself doesn't need one for either — `org-capture`'s file+olp targeting and `org-agenda-files` are both independent of whatever buffer you happen to be looking at.
