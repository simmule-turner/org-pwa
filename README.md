...
- **Properties**: the properties action opens the whole `:PROPERTIES:` drawer as one editable block of `key: value` lines — add a line for a new property, remove a line to delete one, edit a value in place. This is a full replace on save, not a merge: a property you delete from the text stays deleted. When not editing, existing properties show as a small read-only line under the heading, but only when drawers are actually visible for that heading (`#+STARTUP: showeverything`, or manually cycled to "fully expanded" — see [Folding and `#+STARTUP`](#folding-and-startup)); tapping it opens the same action menu as tapping the title.
...
Manually cycling a heading to "fully expanded" (the slide-left gesture, or tapping through the fold states) always reveals everything about that heading — properties and block content included — regardless of what the file's `#+STARTUP:` line says. So a `showall` file still lets you open any specific heading's properties or block content by hand; you're never stuck needing to change the file's own directive just to peek at one thing.
...
- **`org-cycle-open-archived-trees`** — `t` or `nil` (Lisp booleans, not JavaScript truthiness — the string `"true"` is not `t` and won't be treated as one). `nil` (the default, matching real org) means an archived heading (tagged `:ARCHIVE:`) starts folded regardless of the `#+STARTUP:` visibility mode, and refuses to open via cycling — swipe or chevron, whether cascading into it from a parent or acting on it directly. This matches real org-mode's own stated behavior exactly (confirmed against the actual org.el source: the `ARCHIVE` tag's docstring says plainly "An archived subtree does not open during visibility cycling," with no carve-out for which specific action triggered the attempt). Set to `t` to make archived headings behave like any other heading for folding purposes.
...
- **`%%(org-anniversary YEAR MONTH DAY) description`** — real org-mode's own sexp diary-entry function for recurring yearly events (birthdays, anniversaries), written as its own line in a heading's *body* text (not the title — this is a deliberately separate convention from the plain-title-timestamp one above). Recurs every year on MONTH/DAY regardless of YEAR — YEAR only feeds the age calculation, it doesn't gate which years the entry appears in. A `%d` placeholder anywhere in the description is replaced with the elapsed years *as of that specific occurrence* — so the same line correctly shows a different number each year it appears across a multi-year Month or Week view, not one age frozen in for all of them. `YEAR` can be the literal word `nil` when the actual year isn't known; `%d` then substitutes `-1`, matching org-anniversary's own documented behavior for a nil year rather than guessing or omitting it. Shown with a 🎂 icon in the agenda to tell it apart from the other three kinds at a glance. Example: `%%(org-anniversary 1990 5 15) Mom's birthday, turning %d`.
