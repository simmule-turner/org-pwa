...
- **`file:`/`github:`/`webdav:` links are recognized and parsed** — absolute path (`file:/home/user/documents/notes.org`), relative path (`file:projects/todo.org`), a specific headline (`file:~/notes.org::*Project Alpha`), and a text search (`file:~/notes.org::exact phrase`) all parse correctly into a scheme, a path, and an optional in-file target, matching real org's own `::` convention. `github:path/in/repo.org` and `webdav:path/on/server.org` follow the same rules for their own backend. **Tapping one shows what it resolved to but doesn't navigate there yet** — this app is currently single-document, so there's nowhere in the UI for a second file to open into; see [Differences from Emacs org-mode](#differences-from-emacs-org-mode) for the current state of that.
...
- No multi-file switching UI (Agenda and the TODO view are therefore single-file, too — see [Agenda](#agenda)). Capture templates can still target a different file directly (see [Capture Templates](#capture-templates)) without switching what's open — org-capture's own file+olp targeting doesn't require a switching UI either.
