...
- **`showall`** — body text, lists, and properties all shown. Block content (`#+BEGIN_SRC`/`#+BEGIN_QUOTE`/etc.) stays as a collapsed placeholder — matching real org's own distinction between "regular content" and "structural blocks/drawers."
- **`showeverything`** — everything `showall` shows, plus block content itself.

Properties share the same visibility as body text (both tied to whether a heading's body is currently shown) rather than being a separate thing to configure — if you can see a heading's paragraphs and lists, you can see its properties too.

Manually cycling a heading to "fully expanded" (the slide-left gesture, or tapping through the fold states) always reveals everything about that heading — properties and block content included — regardless of what the file's `#+STARTUP:` line says. So a `showall` file still lets you open any specific heading's block content by hand; you're never stuck needing to change the file's own directive just to peek at one thing.
